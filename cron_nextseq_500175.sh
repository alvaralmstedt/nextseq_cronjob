...
then
	...
	#Run bcl2fastq
	nohup bcl2fastq  --runfolder-dir /jumbo/Nextseq500175/$RUN -o ${RUNLOC} -r4 -p4 -d4 -w4 --barcode-mismatches 1 --no-lane-splitting --min-log-level TRACE > ${RUNLOC}/${RUN}_nohup.txt 2>&1 ${RUNLOC}/${RUN}_bcl2fastq.log
	checkExit $? "bcl2fastq"
