...
then
	...
	while [ ! -f ${NS51}/${RUN}/RunCompletionStatus.xml ]
	do
        echo "Sequencing job:$RUN detected but still in progress. Waiting for completion signal."
        sleep 20m
	done	
	STATUSCHECK=$(grep -e "<CompletionStatus>" ${NS51}/${RUN}/RunCompletionStatus.xml | cut -d">" -f2 | cut -d"<" -f1)
	...
	#----------CHECK IF SAMPLESHEET EXISTS-------------->
        ...
        SLEEPCOUNT=0
        while [ ! -f ${NS51}/${RUN}/SampleSheet.csv ]
        do
	...
	EXPERIMENT_NAME=$(grep -e "Experiment Name," ${RUNLOC}/SampleSheet.csv | cut -f2 -d"," | sed 's/\r//')
